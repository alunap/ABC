...
# type 2: right censored (count >= some value, L > 0, U very large/missing)
# type 3: interval censored (L < U, both finite)
...
dat <- dat %>%
  mutate(
    ...
    cens_type = case_when(
      ...
      L > 0 & U > L & U < 10000 ~ 3, # Interval censored
      L >= 1 & U >= 10000 ~ 2, # Right censored (e.g., "6+")
...
ggplot(pred_df, aes(x = temp, y = lambda)) +
  ...
  labs(
    title = "Predicted Bird Counts vs Temperature",
